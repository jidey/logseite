...
<body>
	...
	if ($execute == false)
	{
		echo "<form action=runFeatFailed.php?&url=".urlencode($jenkins)."&Build=".urldecode($Build)."&TestProject=".$TestProject."&JJob=".$JJob."&LogVersion=".$LogVersion."&Testtype=".$Testtype." class=\"form-inline\" role=\"form\">";	
		...
		if(isset($_GET['Confirm']))
		{
			...
			*/
			...
			{			
				...
				/*				
				$selectLastTestRun = "SELECT l1.* FROM `".$LogVersion."` as l1 LEFT JOIN `".$LogVersion."` as l2 
				ON (l1.JParam = l2.JParam AND l1.TCProj = l2.TCProj AND l1.jjob = l2.jjob AND l1.TestLogTyp = l2.TestLogTyp AND l1.Testtype = l2.Testtype AND l1.AutoID < l2.AutoID)
				WHERE ((l1.JJob = '".$JJob."' AND l1.Testtype = '".$Testtype."' AND l1.TCProj = '".$TestListrunrow["TCProj"]."' AND l1.JParam = '".$TestProject."'AND l1.TestLogTyp = 'Single' AND l2.AutoID is NULL))"; 
				*/			
				...
				{	
					...
					{
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x12'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
					else if (substr($LogVersion, 0, 3) == "x13")
					{						
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x13'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
					else if (substr($LogVersion, 0, 3) == "x14")
					{						
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x14'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
					else if (substr($LogVersion, 0, 3) == "x15")
					{						
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x15'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
                    else if (substr($LogVersion, 0, 3) == "x16")
					{						
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x16'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
					else if (substr($LogVersion, 0, 3) == "x17")
					{						
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_x17'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}
					else
					{
						RunTest($_GET['test'],$action,$_GET['branch'],$_GET['Test_Node'],$_GET['LogVersion'],$TestProject,$row["TCProj"],$ID,$_GET['RunBrowser'],$_GET['Confirm'],$_GET['Hub'],$_GET['JJob'],$localrun,$parallel,$Build);
					}						
				}				
			}
    ...
    function RunTest($test,$execute,$branch,$Test_x,$LogVersion,$testset,$JParam,$ID,$Browser,$action,$Hub,$JJob,$localrun,$parallel,$Build)
    {
        if (true)
		{							
			...
			else if (substr($LogVersion, 0, 3) == "x12")
			{
				if($branch == "dev_x12")
					$branch = "devx12";					
				$test = $test."&TestName=".urlencode($JParam)."&Test_Version=".$branch."&TestBrowser=".$Browser."&Test_Node=".$Test_x."&Testset=".$testset."&Hub=".$Hub;							
			}	
